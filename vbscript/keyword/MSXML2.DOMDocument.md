...
### サンプル
```vbscript
' File : ShowXmlNodes.vbs
' Usage : CScript //Nologo ShowXmlNodes.vbs
' Description : VBScript から XML DOM を使用して 人力検索の質問一覧（RSS）のタイトル情報を表示するサンプル。
Option Explicit
...
Sub ShowXmlNodes(Nodes)
    Dim node
    For Each node In Nodes
        If node.parentNode.nodeName = "title" Then
            WScript.Echo node.nodeValue
        End If
